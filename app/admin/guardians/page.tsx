...
async function AdminGuardiansPage() {
  ...
  return (
        ...
        <TableBody>
          {items.map(item => (
            <TableRow key={item.id}>
              <TableCell>
                <Link href={`/guardians/${item.id}`} className="underline font-semibold capitalize">
                  {item.name}
                </Link>
              </TableCell>
              <TableCell>
    ...
    <div className="block sm:hidden space-y-4">
      ...
      <p className="text-center text-muted-foreground mt-4 text-sm">total guardians: {items.length}</p>
    </div>
  </section>
