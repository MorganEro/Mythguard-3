...
function canAccessContract(
  ...
  if (Array.isArray(contractData)) {
    return (
      contractData.some(contract => contract.clerkId === userId) || isAdmin
    );
  }
...
export const fetchAllUsersContracts = async ({
  userId,
}: {
  userId: string;
}) => {
...
export const fetchUsersContracts = async () => {
...
export const fetchSingleContractWithRelatedFields = async (
  contractId: string
) => {
  const { userId } = await auth();
  if (!userId) {
...
export const createContractAction = async (
  ...
  try {
    ...
    if (existingContracts >= MAX_CONTRACTS_PER_USER) {
      return {
        message: `You have reached your maximum of ${MAX_CONTRACTS_PER_USER} contracts.`,
      };
    }
